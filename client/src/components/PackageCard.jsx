...
import { MapPin, Clock, Users, Star, Heart, Utensils, Bed, Car, User } from 'lucide-react'
  ...
  return (
    <Link to={`/packages/${pkg._id}`} className="card group block overflow-hidden">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-3 text-gray-500 mb-4 border-t border-gray-100 pt-3">
          {pkg.inclusions?.some(i => i.toLowerCase().includes('breakfast') || i.toLowerCase().includes('lunch') || i.toLowerCase().includes('dinner') || i.toLowerCase().includes('food')) && (
            <span className="flex items-center gap-1 text-xs bg-gray-100 px-2 py-1 rounded-md" title="Food Included">
              <Utensils className="w-3.5 h-3.5 text-primary-500" />
              Food
            </span>
          )}
          {(pkg.hotel?.name || pkg.hotel?.type) && (
            <span className="flex items-center gap-1 text-xs bg-gray-100 px-2 py-1 rounded-md" title="Hotel Included">
              <Bed className="w-3.5 h-3.5 text-primary-500" />
              Hotel
            </span>
          )}
          {pkg.transport?.type && (
            <span className="flex items-center gap-1 text-xs bg-gray-100 px-2 py-1 rounded-md" title="Transport Included">
              <Car className="w-3.5 h-3.5 text-primary-500" />
              Travel
            </span>
          )}
          {pkg.inclusions?.some(i => i.toLowerCase().includes('guide')) && (
            <span className="flex items-center gap-1 text-xs bg-gray-100 px-2 py-1 rounded-md" title="Guide Included">
